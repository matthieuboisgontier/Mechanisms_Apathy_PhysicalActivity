...
# 95 confidence level for apathy total score
describe(DataAggreg$lars_sum)
z <- qnorm(1 - (0.05 / 2)) # Z-score for 95% CI
# Confidence interval
lower_bound <- 31.25 - z * 0.48
upper_bound <- 31.25 + z * 0.48
ci_lars_sum <- c(lower_bound, upper_bound)
ci_lars_sum

# 95 confidence level for apathy item score
describe(DataAggreg$larsmean)
# Confidence interval
lower_bound_item <- 2.89 - z * 0.05 # mean_value - z * std_error
upper_bound_item <- 2.89 + z * 0.05
ci_lars_item <- c(lower_bound_item, upper_bound_item)
ci_lars_item
