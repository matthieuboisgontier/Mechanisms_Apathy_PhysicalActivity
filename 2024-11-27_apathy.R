...
# read in data ------
data1 <- read.csv(here("data-apathy", "2024-05-12_data_apathy.3.csv"))
...
cleanlightdata1 <- cleandata1[cleandata1$trialcode != "fixation" & 
                                ...
                                # remove 15 first trials of the study
                                      ...
                                      #& cleandata1$correct != "0" #to know the number of errors
                                      , ] 
count(cleanlightdata1)
unique (cleanlightdata1$sex)
unique(cleanlightdata1$id)
var(cleanlightdata1$latency)
...
sum(with(cleanlightdata1, geomfigure == "1"))

cleanlightdata1$onlysed = (cleanlightdata1$trialcode!="ApApproach_ManikinTop" &
                              cleanlightdata1$trialcode!="ApApproach_ManikinBottom" &
                              cleanlightdata1$trialcode!="ApAvoid_ManikinBottom" &
                              cleanlightdata1$trialcode!="ApAvoid_ManikinTop" &
                              cleanlightdata1$trialcode!="squareApproach_ManikinTop" & 
                              cleanlightdata1$trialcode!="circleAvoid_ManikinBottom" &
                              cleanlightdata1$trialcode!="circleAvoid_ManikinTop" &
                              cleanlightdata1$trialcode!="squareApproach_ManikinBottom" &
                              cleanlightdata1$trialcode!="circleApproach_ManikinBottom" &
                              cleanlightdata1$trialcode!="squareAvoid_ManikinBottom" &
                              cleanlightdata1$trialcode!="circleApproach_ManikinTop" &
                              cleanlightdata1$trialcode!="squareAvoid_ManikinTop")+0
select(cleanlightdata1, trialcode, onlysed)
sum(with(cleanlightdata1, onlysed == "1"))

cleanlightdata1$onlyap = (cleanlightdata1$trialcode!="SedenApproach_ManikinTop" &
                        cleanlightdata1$trialcode!="SedenApproach_ManikinBottom" &
                        cleanlightdata1$trialcode!="SedenAvoid_ManikinBottom" &
                        cleanlightdata1$trialcode!="SedenAvoid_ManikinTop" &
                        cleanlightdata1$trialcode!="squareApproach_ManikinTop" & 
                        ...
                        cleanlightdata1$trialcode!="circleApproach_ManikinTop" &
                        cleanlightdata1$trialcode!="squareAvoid_ManikinTop")+0
select(cleanlightdata1, trialcode, onlyap)
sum(with(cleanlightdata1, onlyap == "1"))
...
data2$sum_chronic_c <- scale (data2$sum_chronic, center = TRUE, scale = TRUE)
...
# aggregate data per subject
DataAggreg = aggregate (cbind(kpsum, attitude,intention, pain, age, sex01, mvpa, gender01,
                              bmi, sum_chronic, moderatepa, vigorouspa, biassedcorr, biasapcorr, larsmean, dassmean, lars_sum, dass_sum, attitude1, attitude2)
                        ~id, data=data2, FUN=mean)

#correlations
cor.test(cleanlightdata1$attitude1, cleanlightdata1$attitude2, method = "pearson")
cor.test(DataAggreg$age, DataAggreg$lars_sum, method = "pearson")
plot(DataAggreg$age, DataAggreg$lars_sum)
cor.test(DataAggreg$sum_chronic, DataAggreg$lars_sum, method = "pearson")
plot(DataAggreg$sum_chronic, DataAggreg$lars_sum)

describe(DataAggreg$age)
describe(DataAggreg$intention)
describe(DataAggreg$sum_chronic)
describe(DataAggreg$dass_sum)
describe(DataAggreg$attitude)
describe (DataAggreg$lars_sum)
sum(with(DataAggreg,lars_sum > "20"))
sum(with(DataAggreg,lars_sum > "33"))

# 95 confidence level for apathy item score for sample without any chronic condition
subset_nochronic <- subset(DataAggreg, sum_chronic == 0)
describe (subset_nochronic$larsmean)
z <- qnorm(1 - (0.05 / 2)) # Z-score for 95% CI
lower_bound_item_nochronic <- 2.93 - z * 0.07 # mean_value - z * std_error
upper_bound_item_nochronic <- 2.93 + z * 0.07
ci_lars_item_nochronic <- c(lower_bound_item_nochronic, upper_bound_item_nochronic)
ci_lars_item_nochronic

# 95 confidence level for apathy item score for full sample
describe(DataAggreg$larsmean)
z <- qnorm(1 - (0.05 / 2)) # Z-score for 95% CI
lower_bound_itemfull <- 2.89 - z * 0.05 # mean_value - z * std_error
upper_bound_itemfull <- 2.89 + z * 0.05
ci_lars_itemfull <- c(lower_bound_item, upper_bound_item)
ci_lars_itemfull

# test correlation between age and apathy in healthy participants
cor.test(subset_nochronic$age, subset_nochronic$lars_sum, method = "pearson")
plot(subset_nochronic$age, subset_nochronic$lars_sum)
...
DataAggreg$sum_chronic_c <- scale (DataAggreg$sum_chronic, center = TRUE, scale = TRUE)
...
mediation.1 <- lm (intention ~  lars_sum_c + dass_sum_c + biasapcorr_c + biassedcorr_c + age_c + sex01 + bmi_c + sum_chronic_c, data=DataAggreg, na.action=na.omit) 
summary(mediation.1)
confint(mediation.1)
plot(allEffects(mediation.1), select = 1)

mediation.2 <- lm (attitude ~ lars_sum_c  + dass_sum_c + biasapcorr_c + biassedcorr_c + age_c + sex01 + bmi_c + sum_chronic_c, data=DataAggreg, na.action=na.omit) 
summary(mediation.2)
confint(mediation.2)
plot(allEffects(mediation.2), select = 1)

mediation.3 <- lm (intention ~ attitude_c + lars_sum_c  + dass_sum_c + biasapcorr_c + biassedcorr_c  + age_c + sex01 + bmi_c + sum_chronic_c, data=DataAggreg, na.action=na.omit) 
summary(mediation.3)
confint(mediation.3)
...
mediation.4 <- lm (mvpa ~ lars_sum_c + dass_sum_c + biasapcorr_c + biassedcorr_c + age_c + sex01 + bmi_c + sum_chronic_c, data=DataAggreg, na.action=na.omit) 
summary(mediation.4)
confint(mediation.4)
plot(allEffects(mediation.4), select = 1)

mediation.5 <- lm (intention ~ lars_sum_c  + dass_sum_c + biasapcorr_c + biassedcorr_c + age_c + sex01 + bmi_c + sum_chronic_c, data=DataAggreg, na.action=na.omit) 
summary(mediation.5)
confint(mediation.5)
plot(allEffects(mediation.5), select = 1)

mediation.6 <- lm (mvpa ~ intention_c + lars_sum_c  + dass_sum_c + biasapcorr_c + biassedcorr_c  + age_c + sex01 + bmi_c + sum_chronic_c, data=DataAggreg, na.action=na.omit) 
summary(mediation.6)
confint(mediation.6)
...
lmm1.1 <- lmer(relativelatencygeomdirection  ~ 1 + approach*stimulus_ap0_sed1*lars_sum_c + dass_sum_c + attitude_c + mvpa_c + age_c + sex01 + bmi_c + computer01 + sum_chronic_c + (1|id) + (1|pictograms), 
...
lmm1.2 <- lmer(relativelatencygeomdirection  ~ 1 + avoid.1.approach.0*stimulus_ap0_sed1*lars_sum_c + dass_sum_c + attitude_c + mvpa_c + age_c + sex01 + bmi_c + computer01 + sum_chronic_c +  (1|id) + (1|approach)  + (1|pictograms), 
             data=data2, subset = (stimulus == "sed"| stimulus == "ap") & error == 0, REML=T, na.action=na.omit)
summary(lmm1.2) 
#confint(lmm1.2)
...
lmm1.3 <- lmer(relativelatencygeomdirection  ~ 1 + approach*stimulus_ap1_sed0*lars_sum_c + dass_sum_c + attitude_c + mvpa_c + age_c + sex01 + bmi_c + computer01 +sum_chronic_c + (1|id) + (1|approach) + (1|pictograms), 
               data=data2, subset = (stimulus == "sed"| stimulus == "ap")  & error == 0, REML=T, na.action=na.omit)
summary(lmm1.3)
#confint(lmm1.3)
...
lmm1.4 <- lmer(relativelatencygeomdirection  ~ 1 + avoid.1.approach.0*stimulus_ap1_sed0*lars_sum_c + dass_sum_c + attitude_c + mvpa_c + age_c + sex01 + bmi_c + computer01 + sum_chronic_c + (1|id) + (1|pictograms), 
               data=data2, subset = (stimulus == "sed"| stimulus == "ap")  & error == 0, REML=T, na.action=na.omit)
summary(lmm1.4) 
#confint(lmm1.4)
